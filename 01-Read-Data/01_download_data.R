...
list_of_Weather_sheets <- gs_ls("Weather")[c(1,3,6)]

# select sheets for download 
list_of_Weather_sheets_to_download <-
  list_of_Weather_sheets %>%
  select(sheet_title) %>%
  # select sites with CD treatment
...
for (i in seq_along(list_of_Weather_sheets_to_download)) {
  print(paste("downloading", list_of_Weather_sheets_to_download[i], "..."))
  gs_download(from = gs_title(list_of_Weather_sheets_to_download[i]),
              to = paste0("Data/On-Site-Weather/", list_of_Weather_sheets_to_download[i], ".xlsx"),
              overwrite = TRUE)
}
